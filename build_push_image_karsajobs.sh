...
docker build -t archieops/karsajobs:latest .
...
docker tag archieops/karsajobs:latest ghcr.io/archieops/karsajobs:latest
# Login ke ghcr.io
echo $GHCRTOKEN | docker login ghcr.io --username ArchieOps --password-stdin
# Push image ke ghcr.io
docker push ghcr.io/archieops/karsajobs:latest